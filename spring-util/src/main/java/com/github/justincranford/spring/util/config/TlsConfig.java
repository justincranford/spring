...
public class TlsConfig {
    ...
    public record TlsAutoGeneratedConfig(
    ...
    public ServletWebServerFactory servletWebServerFactory(
        ...
        final TlsAutoGeneratedConfig tlsAutoGeneratedConfig
    ) throws Exception {
        // use ${server.ssl.*} settings to start Tomcat with auto-generated TLS server PEM files
        final TomcatServletWebServerFactory factory = new TlsTomcatServletWebServerFactory(tlsAutoGeneratedConfig);
    ...
    public class TlsTomcatServletWebServerFactory extends TomcatServletWebServerFactory {
        ...
        private final TlsAutoGeneratedConfig tlsAutoGeneratedConfig;

        public TlsTomcatServletWebServerFactory(final TlsAutoGeneratedConfig tlsAutoGeneratedConfig) {
        	this.tlsAutoGeneratedConfig = tlsAutoGeneratedConfig;
        }
        ...
        public void customizeConnector(final Connector connector) {
        	if (TlsConfig.this.serverSslAutoGenerateCertificates) {
                try { // Replace server.ssl.* properties in memory, pointing to the temp PEM files
                    ...
                    ssl.setTrustCertificate(this.tlsAutoGeneratedConfig.caCertificatePath().toFile().toString());
                    ssl.setCertificate(this.tlsAutoGeneratedConfig.serverCertificatePath().toFile().toString());
                    ssl.setCertificatePrivateKey(this.tlsAutoGeneratedConfig.serverPrivateKeyPath().toFile().toString());
                    this.setSsl(ssl);
                } catch(Exception e) {
    ...
    // TODO @Conditional @Value(value="${server.ssl.auto-generate-certificates:false}")
    @Bean
    public TlsAutoGeneratedConfig tlsServerChain() throws Exception {
        ...
        final Certificate serverCert = createCert(
            ...
            new Extensions(new Extension[] {
                ...
                new Extension(Extension.subjectAlternativeName, false, new GeneralNames(new GeneralName[] {
                	new GeneralName(GeneralName.dNSName, "localhost"), new GeneralName(GeneralName.dNSName, "127.0.0.1"), new GeneralName(GeneralName.dNSName, "::1")
        ...
        return new TlsAutoGeneratedConfig(
    ...
    // TODO Conditional     @Value(value="${server.ssl.auto-generate-certificates:false}")
    @Bean
	public SSLContext clientSslContext(final TlsAutoGeneratedConfig tlsAutoGeneratedConfig) {
